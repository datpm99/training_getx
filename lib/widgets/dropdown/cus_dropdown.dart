...
/// which is dropdown button's drop down duration.
const Duration _kDropdownMenuDuration = Duration(milliseconds: 300);

/// which is dropdown item's default height
const double _kMenuItemHeight = 48.0;

/// which is dropdown item's default padding.
const EdgeInsets _kMenuItemPadding = EdgeInsets.symmetric(horizontal: 16.0);

/// which is dropdown item's default margin
const EdgeInsets _kAlignedMenuMargin = EdgeInsets.zero;

/// which is dropdown item's default margin for align rule.
        ...
        child: InkWell(
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border(
                top: BorderSide(width: 0.5, color: Color(0xffe7e9ef)),
                bottom: (itemIndex == route.items.length - 1)
                    ? BorderSide(width: 0.5, color: Color(0xffe7e9ef))
                    : BorderSide.none,
              ),
            ),
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    return Offset(left + 8 + pLeft, menuTop + 13);
  }
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    final double preferredMenuHeight =
        (4 * _kMenuItemHeight) + kMaterialListPadding.vertical;
...
class DropdownBelow<T> extends StatefulWidget {
  DropdownBelow({
    ...
    this.name = '',
    this.pLeft = 0,
    this.bgColorDropdown = Colors.white,
    this.isBorder = true,
    this.iconSelected = Icons.keyboard_arrow_up,
    this.isCusItemSelected = false,
    this.isCusItemSelectedTask = false,
    this.widgetCusItem = const SizedBox(),
    this.icon,
    this.iconActive,
    this.onTapDropdown,
  })  : assert(value == null ||
  ...
  final bool isCusItemSelected;

  final bool isCusItemSelectedTask;

  final Widget widgetCusItem;
...
class _DropdownBelowState<T> extends State<DropdownBelow<T>>
  ...
  Widget build(BuildContext context) {
    ...
    Widget result = DefaultTextStyle(
      ...
      child: Container(
        decoration: BoxDecoration(
          color: widget.bgColorDropdown,
          border: (widget.isBorder)
              ? Border.all(
                  width: 0.5,
                  color: isOpenDrop ? Styles.primaryColor : Styles.grey12,
                )
              : null,
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: (_selectedIndex == null)
                        ...
                        : (widget.isCusItemSelected)
                            ? (!widget.isCusItemSelectedTask)
                                ? IndexedStack(
                                    index: _selectedIndex ?? hintIndex,
                                    alignment: AlignmentDirectional.centerStart,
                                    children: items,
                                  )
                                : widget.widgetCusItem
                            : Text(
                                widget.name,
                                style: widget.boxTextstyle,
                                maxLines: 1,
